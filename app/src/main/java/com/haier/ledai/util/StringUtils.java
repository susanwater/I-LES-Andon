...
public class StringUtils {
	...
	/** (0-9中文数字) */
	private final static String[] figDecode = {"零", "一", "二", "三", "四", "五",
			"六", "七", "八", "九"};
	...
	/**
	 * @param many
	 * @return 返回many位的随即字符串
	 */
	public static String randomNum(int many){
	...
	public static String formatThousandOne(double number) {
	 ...
	 * @return 经过转换得到的四位小数
	 */
	public static float getNumber4Decimal(double number) {
		float f = Float.valueOf(formatDouble(number, "0.0000"));
		return f;
	}
	...
	public static String blurString(String st) {
		if (isEmpty(st))
			return "";
		...
		return st.substring(0, 3) + "****" + st.substring(len - 4, len);
	}
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	public static String replaceStringToBlur(String st) {
		if (isEmpty(st))
			return "";
		...
		string.append(st.substring(st.length() - 1, st.length() - 1));
